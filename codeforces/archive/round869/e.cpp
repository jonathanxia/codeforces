...
ll coeff(vl& a, ll d, const vl& facs) {
    ll tot = 0;
    rep(i, 0, d) {
        // (i - 0)(i - 1) * ... * (i - d)
        ll sgn = (d - i) % 2 ? 1 : -1;
        ll denom = nt::mod(facs[i] * sgn);
        ll num = nt::mod(-d * (d + 1) / 2 - i);
        tot += nt::mdiv(num, denom) * a[i];
        tot = nt::mod(tot);
    }
    return tot;
}

ll coeff0(vl& a, ll d, const vl& facs) {
    ll tot = 0;
    ll sgn = (d % 2 == 0) ? 1 : -1;
    rep(i, 0, d) {
        sgn *= -1;
...
void solve() {
    ...
    facs[0] = 1;
    rep(i, 1, d + 1) {
        facs[i] = nt::mod(facs[i - 1] * i);
    }

    vl facs2(d + 1);
    rep(i, 0, d) {
        ass(facs2[i], facs2[d - i], facs[i] * facs[d - i], facs[i] * facs[d - i]);
    }

    ll a0 = coeff0(a, d, facs2);
    rep(i, 0, d) {
        a[i] -= b[i];
    }
    ll delta = coeff(a, d, facs2);
    print(nt::mod(nt::mdiv(delta, a0 * d)));
}
