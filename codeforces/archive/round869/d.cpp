    ...
    rep(v, 0, n - 1) {
        ...
        auto dfs = [&](auto&& dfs, ll node) -> void {
            ...
            foreach(child, graph[node]) {
                ...
                if (child == v && parent[node] != v) {
                    endpoint = node; // Found a cycle!
                }
            }
        };
        ...
        // Should be possible now, just need to output the
        // actual fish
        print("YES");

        vector<pl> output;

        uset neighbors(graph[v].begin(), graph[v].end());
        ll ptr = parent[endpoint];
        neighbors.erase(endpoint);
        output.pb({v + 1, endpoint + 1});
        output.pb({endpoint + 1, ptr + 1});

        while (ptr != v) {
            if (neighbors.count(ptr)) {
                output.pb({ptr + 1, v + 1});
                neighbors.erase(ptr);
                break;
            }
            output.pb({ptr + 1, parent[ptr] + 1});
            ptr = parent[ptr];
        }
