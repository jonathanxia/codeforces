...
void solve() {
    ...
    vv::sort(intervals, 0, n - 1, [](pl p) -> pl { return {p.second, p.first}; });

    vl max_size(n + 1);
    // max_size[i] = intervals up to and including i - 1
    rep(i, 0, n - 1) {
        // Do I take the i-th interval or not?

        // If do not take, then just previous value
        chkmax(max_size[i + 1], max_size[i]);

        // If do take, then find a partner
        ll best_left = -100;
        dep(j, i - 1, 0) {
            if (intervals[j].second < intervals[i].first) {
                break;
            }
            chkmax(best_left, intervals[j].first);
        }
        if (best_left < 0) {
            // No partner found, cannot take
            continue;
        }

        ll left = min(best_left, intervals[i].first);
        ll idx = largest_st(x, intervals[x].second < left, 0, i);

        chkmax(max_size[i + 1], max_size[idx + 1] + 2);
    }


    // Claim: if you have to take rightmost, always pair it with
    // guy with most right left bracket.

    // Reminder! Subtract max from n;
    print(n - max_size.back());
}
