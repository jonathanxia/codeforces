...
void solve() {
    ...
    vl cat = combo::precompute_catalan(n);
    
    // Compute a random xor for each interval
    vl iid(k);
    rep(i, 0, k - 1) {
        iid[i] = uid(0, LONG_LONG_MAX);
    }
    ...
    ll mask = 0;
    rep(i, 0, n - 1) {
        while (idx < 2 * k && points[idx].first == i) {
            // Apply masks
            ll interval = points[idx].second;
            mask ^= iid[interval];
            idx++;
        }
        cover[i] = mask;
    ...
    ll p = 1;
    auto groups = vv::counter(cover);
    foreachp(zz, v, groups) {
        if (v % 2 == 1) {
            p *= 0;
        }
        else {
