...
ll solve(ll n) {
    ...
    rep(i, 0, n - 1) {
        ...
        left_cummax_idx[i] = best_idx;
    }

    best = -pow(10, 9);
    vl right_cummax_idx(n);
    dep(i, n - 1, 0) {
        if (b[i] - i > best) {
            best = b[i] - i;
            best_idx = i;
        }
        right_cummax_idx[i] = best_idx;
    }

    ll best_score = -1;
    ll new_score;
    rep(i, 1, n - 2) {
        int L = left_cummax_idx[i - 1];
        int R = right_cummax_idx[i + 1];
        new_score =b[L] + b[i] + b[R] - (R - L);
