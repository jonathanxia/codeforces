...
void solve() {
    ...
    vl lengths = RC(vl, rights[i] - lefts[i] + 1, i, 0, n - 1);
    ll longest_idx = vv::argmax(lengths);

    ll ans = 0;
    rep(i, 0, n - 1) {
        // Check each interval
        ll L = lefts[longest_idx];
        ll R = rights[longest_idx];
        if (L <= lefts[i] && rights[i] <= R) {
            chkmax(ans, 2 * (R - L - rights[i] + lefts[i]));
        }
    }

    ll max_left = vv::max(lefts);
    ll min_right = vv::min(rights);

    rep(i, 0, n - 1) {
        // For each interval just find who intersects with me the least
        ll length = rights[i] - lefts[i] + 1;
        if (max_left > rights[i]) {
            chkmax(ans, 2 * length);
            continue;
        }
        if (min_right < lefts[i]) {
            chkmax(ans, 2 * length);
            continue;
        }

        // Someone intersects with me? Shucks, take the largest one
        chkmax(ans, 2 * (max_left - lefts[i]));
        chkmax(ans, 2 * (rights[i] - min_right));
    }
