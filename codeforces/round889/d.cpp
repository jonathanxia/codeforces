...
void solve() {
    ...
    achieve[0] = true;
    rep(i, 0, n - 1) {
        // What is achievable with the i-th guy is
        // the current achievements, but along with
        // the current achievements + a[i]
        achieve |= (achieve >> i) << (a[i] + i);
    }
    vl c = cumsum(a);
    rep(i, n, 2 * n) {
        c.pb(c[n - 1]);
