...
void solvePos(vl& arr, vpl& approach);
void solveNeg(vl& arr, vpl& approach);
void solve()
{
    ll n;
    cin >> n;
    ...
    ll minidx = vv::argmin(arr);
    ll maxidx = vv::argmax(arr, false);
    ll mini = arr[minidx];
    ll maxi = arr[maxidx];

    dprint("MAXI", maxi, "MINI", mini);
    if (mini == maxi) {
        print(0);
        return;
    }
    ...
    // do posApproach
    if (maxi > 0) {
        // keep doubling maxi until it's greater than -mini
        while (posMaxi < -mini) {
            posApproach.pb({ maxidx, maxidx });
            posArr[maxidx] += posArr[maxidx];
            posMaxi *= 2;
        }
        // then add maxi to everything that is negative
        walk(i, posArr)
        {
            if (posArr[i] < 0) {
                posApproach.pb({ i, maxidx });
                posArr[i] += posArr[maxidx];
            }
        }
        solvePos(posArr, posApproach);
        dprint(posArr);
    }
    // do negApproach
    if (mini < 0) {
        // keep doubling mini until it's less than -maxi
        while (negMini > -maxi) {
            negApproach.pb({ minidx, minidx });
            negArr[minidx] *= 2;
            negMini *= 2;
        }
        // then add mini to everything positive
        walk(i, negArr)
        {
            if (negArr[i] > 0) {
                negApproach.pb({ i, minidx });
                negArr[i] += negArr[minidx];
            }
        }
        solveNeg(negArr, negApproach);
        dprint("negArr", negArr);
    }
    if (!negApproach.empty() && len(negApproach) < len(posApproach)) {
        dprint("neg");
        print(len(negApproach));
        foreach (op, negApproach) {
            print(op.first + 1, op.second + 1);
        }
    } else {
        dprint("pos");
        print(len(posApproach));
        foreach (op, posApproach) {
            print(op.first + 1, op.second + 1);
        }
    }
}
