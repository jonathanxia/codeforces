...
void solve() {
    ...
    rep(i, 0, n - 1) {
        ...
        if (is_strong) {
            strong_segments.pb(mp(i, end - 1));
        }
        else {
            weak_segments.pb(mp(i, end - 1));
        }
    }

    ll ans = len(weak_segments) + len(strong_segments);
    // How many unsaved zeros?
    rep(j, 0, len(strong_segments) - 1) {
        pl elem = strong_segments[j];
        if (ordered(0, elem.first - 1, n - 1)) {
            unmarked_zero[elem.first - 1] = false;
        }
        if (ordered(0, elem.second + 1, n - 1)) {
            unmarked_zero[elem.second + 1] = false;
        }
    }
    // Can we save anybody else with weak segments?
    rep(j, 0, len(weak_segments) - 1) {
        pl elem = weak_segments[j];
        if (ordered(0, elem.first - 1, n - 1) && unmarked_zero[elem.first - 1]) {
            unmarked_zero[elem.first - 1] = false;
            continue;
        }
        if (ordered(0, elem.second + 1, n - 1) && unmarked_zero[elem.second + 1]) {
            unmarked_zero[elem.second + 1] = false;
            continue;
        }
    }

    ll extra = vv::sum(unmarked_zero);
    print(ans + extra);
}
