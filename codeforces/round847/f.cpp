...
ll shortest_to_black(const vb& is_black, ll n, ll node, const vvl& graph, ll cur_best, vl& dist) {
    vb visited(n + 1);

    auto dfs = [&](auto&& dfs, ll v, ll cur_dist) -> void {
        if (cur_dist >= cur_best) {
            // Stop looking, there's no point
            return;
        }
        dist[v] = min(cur_dist, dist[v]);
        visited[v] = true;
        foreach(child, graph[v]) {
            if (visited[child]) {
                continue;
            }

            if (dist[child] >= cur_dist + 1) {
                dist[child] = cur_dist + 1;
                dfs(dfs, child, cur_dist + 1);
            }
            else {
                cur_best = min(cur_best, dist[child] + cur_dist + 1); 
            }
        }
    };
...
void solve() {
    ...
    ll best = INT_MAX;
    vl dist_cache(n + 1);
    // Just populate it real fast
    vb visited(n + 1);
    auto mydfs = [&](auto&& mydfs, ll node, ll cur) -> void {
        dist_cache[node] = cur;
        visited[node] = true;
        foreach(child, graph[node]) {
            if (!visited[child]) {
                mydfs(mydfs, child, cur + 1);
            }
        }
    };
    ...
    rep(i, 1, n - 1) {
        ...
        ll dist = shortest_to_black(is_black, n, bl_seq[i], graph, best, dist_cache);
        best = min(best, dist);
        answers.push_back(best);

        is_black[bl_seq[i]] = true;
    }
    print(answers);
}
