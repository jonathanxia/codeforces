...
ll shortest_to_black(const vb& is_black, ll n, ll node, const vvl& graph, ll cur_best) {
    vb visited(n + 1);
...
void solve() {
    ...
    rep(i, 1, n - 1) {
        ...
        ll dist = shortest_to_black(is_black, n, bl_seq[i], graph, best);
        best = min(best, dist);
        answers.push_back(best);
