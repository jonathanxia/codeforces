...
void solve() {
    ...
    int g = a[0];
    FOR(i,0,n-1) g = gcd(g, a[i]);

    int inf = 99999999;
    ndarray<int,2,false> dp{{n,5001}, inf};
    FOR(i,0,n-1) dp(i, a[i]) = 0;
    DOR(i,n-1,0) {
        FOR(v,1,5000) {
            chkmin(dp(i, v), dp(i+1, v));
        }
        FOR(v,1,5000) {
            if (dp(i+1,v) >= inf) continue;
            int z = gcd(a[i], v);
            chkmin(dp(i, z), 1 + dp(i+1, v));
        }
    }

    int k = dp(0, g);
    int ans = k;
    FOR(i,0,n-1) if(a[i] != g) ans += 1;
    // we already converted one guy
    if (k > 0) ans -= 1;
    print(ans);
}