...
void solve() {
    // print("START");
    int n,m; read(n,m);
    vvl add_events(n+2), rem_events(n+2);
    FOR(i,1,m) {
        int l,r; read(l,r);
        // print("n,l,r",n,l,r);
        add_events[r].push_back(r);
        // print("l", l); cout << flush;
        rem_events[l-1].push_back(r);
    }
    // print("END");
    vl dp(n+2);
    dp[n+1] = 0;
    multiset<int> cats;
    DOR(i,n,0) {
        foreach(r, rem_events[i]) cats.erase(cats.find(r));
        foreach(r, add_events[i]) cats.insert(r);
        dp[i] = dp[i+1];
        if(!cats.empty()) {
            chkmax(dp[i], len(cats) + dp[*cats.rbegin() + 1]);
        }
    }
    print(dp[0]);
}