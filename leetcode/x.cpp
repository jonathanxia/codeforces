...
#define METHOD longestValidSubstring
...
public:
    int longestValidSubstring(string word, vector<string>& forbidden) {
        ...
        ll n = len(word);
        vl badness(n, INT_MAX);
        rep(i, 0, n - 1) {
            rep(m, 1, 10) {
                string chk = str::slc(word, i, i + m);
                if (bad.count(chk)) {
                    chkmin(badness[i], m);
                    break;
                }
        ...
        // For each guy, ask how far can he go?
        vl farthest(n);
        rep(i, 0, n - 1) {
            farthest[i] = min(n, i + badness[i] - 1);
        }
        vl reach = vv::cummin(farthest, true);
        ll ret = 0;
        rep(i, 0, n - 1) {
            chkmax(ret, reach[i] - i);
        }
        return ret;
    }
};
...
int main() {
    Solution s;
    string y = "leetcode";
    vector<string> forbid = {"de", "le", "e"};
    // string y = "aaaa";
    // vector<string> forbid = {"a"};
    print(s.METHOD(y, forbid));
    return 0;
}
#endif