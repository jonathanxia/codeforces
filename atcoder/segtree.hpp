...
template <class S, S (*op)(S, S), S (*e)()> struct segtree {
  ...
  public:
    ...
    */
    ...
    template <class F> int largest_right_st(int left, const F& f) const {
        return this->max_right(left, f) - 1;
    }

    // Returns the smallest right such that
    // f(segtree.query(left, right)) = true
    template <class F> int smallest_right_st(int left, const F& f) const {
        return this->max_right(left, [&](S x) { return !f(x); });
    }

    // Returns the smallest left such that
    // f(segtree.query(left, right)) = true
    template <class F> int smallest_left_st(int right, const F& f) const {
        return this->min_left(right + 1, f);
    }

    // Returns the largest left such that
    // f(segtree.query(left, right)) = true
    template <class F> int largest_left_st(int right, const F& f) const {
